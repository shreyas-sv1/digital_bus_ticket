...
export class RoutesService {
  ...
  async addStop(routeId: string, dto: CreateStopDto) {
    ...
    if (existing)
      throw new BadRequestException(
        `Stop order ${dto.stopOrder} already exists on this route`,
      );
  ...
  async calculateFare(
    ...
    if (exactFare)
      return { fare: exactFare.amount, fromStopOrder, toStopOrder };
  ...
  /**
   * Searches routes by route name or stop name.
   * Returns matching routes with their stops and the buses currently assigned
   * to each route so the traveller can identify the right bus number.
   */
    ...
    return this.prisma.route.findMany({
      where: {
        OR: [
          ...
          { stops: { some: { stopName: { contains: q } } } },
        ],
      },
      include: {
        stops: { orderBy: { stopOrder: 'asc' } },
